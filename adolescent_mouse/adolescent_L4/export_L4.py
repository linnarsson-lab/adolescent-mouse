...
class ExportL4(luigi.Task):
	...
	def requires(self) -> List[luigi.Task]:
		return [
			am.AggregateL4(target=self.target),
			am.CurateL4(target=self.target)
		]

	def output(self) -> luigi.Target:
		return luigi.LocalTarget(os.path.join(am.paths().build, "L4_" + self.target + "_exported"))
	...
	def run(self) -> None:
		...
		with self.output().temporary_path() as out_dir:
			...
			dsagg.export(os.path.join(out_dir, "L4_" + self.target + "_expression.tab"))
			dsagg.export(os.path.join(out_dir, "L4_" + self.target + "_enrichment.tab"), layer="enrichment")
			dsagg.export(os.path.join(out_dir, "L4_" + self.target + "_enrichment_q.tab"), layer="enrichment_q")
			dsagg.export(os.path.join(out_dir, "L4_" + self.target + "_trinaries.tab"), layer="trinaries")
			...
			cg.plot_graph(ds, os.path.join(out_dir, "L4_" + self.target + "_manifold.aa.png"), tags)
			...
			cg.plot_graph(ds, os.path.join(out_dir, "L4_" + self.target + "_manifold.aaa.png"), tags)

			logging.info("Plotting marker heatmap")
			cg.plot_markerheatmap(ds, dsagg, n_markers_per_cluster=self.n_markers, out_file=os.path.join(out_dir, "L4_" + self.target + "_heatmap.pdf"))
			...
			with open(os.path.join(out_dir, "L4_" + self.target + "_distances.txt"), "w") as f:
