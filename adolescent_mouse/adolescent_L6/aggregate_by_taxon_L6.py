...
class AggregateByTaxonL6(luigi.Task):
	"""
	Aggregate by taxon in a new Loom file
	"""
	...
	def requires(self) -> List[luigi.Task]:
		return am.ExtractByTaxonL6(rank=self.rank)

	def output(self) -> luigi.Target:
		return luigi.LocalTarget(os.path.join(am.paths().build, f"L6_R{self.rank}.agg.loom"))
